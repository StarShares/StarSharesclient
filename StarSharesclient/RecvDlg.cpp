...
void CRecvDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EXPORT_EXCEL, m_rBtnExcel);
}

BEGIN_MESSAGE_MAP(CRecvDlg, CDialogBar)
	ON_WM_ERASEBKGND()
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_MESSAGE(MSG_USER_RECIVE_UI , &CRecvDlg::OnShowListCtrl )
	ON_BN_CLICKED(IDC_BUTTON_JHDZ, &CRecvDlg::OnBnClickedButtonJhdz)
...
// CRece 消息处理程序
void CRecvDlg::SetBkBmpNid( UINT nBitmapIn ) 
{
	if( NULL != m_pBmp ) {
		::DeleteObject( m_pBmp ) ;
		m_pBmp = NULL ;
...
BOOL CRecvDlg::OnEraseBkgnd(CDC* pDC)
{
...
int CRecvDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL CRecvDlg::Create(CWnd* pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID)
{
	...
	if ( bRes ) {
		...
		m_listCtrl.SetHeaderBKColor(87, 101, 112, 0); 
		...
		m_rBtnExcel.SetBitmaps( IDB_BITMAP_BUT2, RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
		...
		m_rBtnAcitve.SetBitmaps( IDB_BITMAP_BUT2 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
		...
		m_rBtnNewaddr.SetBitmaps( IDB_BITMAP_BUT2 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
		...
		m_rBtnCopyaddr.SetBitmaps( IDB_BITMAP_BUT2 , RGB(255, 255, 0) , IDB_BITMAP_BUT1 , RGB(255, 255, 255) );
...
void CRecvDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if( NULL != GetSafeHwnd() ) {
		...
		if ( NULL != pWnd ) {
			pWnd->SetWindowPos( NULL ,30 , 100 , 840, 330  ,SWP_SHOWWINDOW ) ; 
		}

		pWnd = (CWnd*)GetDlgItem(IDC_EXPORT_EXCEL);
		if ( NULL != pWnd ) {
			pWnd->GetWindowRect(rc);
			pWnd->SetWindowPos( NULL ,30 , 36 , rc.Width(), rc.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_JHDZ);
		if ( NULL != pWnd ) {
			pWnd->GetWindowRect(rc);
			pWnd->SetWindowPos( NULL ,140 , 36 , rc.Width(), rc.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_BUTTON_NEWADDRESS);
		if ( NULL != pWnd ) {
			pWnd->GetWindowRect(rc);
			pWnd->SetWindowPos( NULL ,250 , 36 , rc.Width(), rc.Height()  ,SWP_SHOWWINDOW ) ; 
		}
		pWnd = (CWnd*)GetDlgItem(IDC_COPYADDRESS);
		if ( NULL != pWnd ) {
			pWnd->GetWindowRect(rc);
			pWnd->SetWindowPos( NULL ,360 , 36 , rc.Width(), rc.Height()  ,SWP_SHOWWINDOW ) ; 
		}
...
void CRecvDlg::OnBnClickedButtonJhdz()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CRecvDlg::OnBnClickedButtonNewaddress()
{
	// TODO: 在此添加控件通知处理程序代码
	CNewAddressDlg dlg;
	dlg.DoModal();
...
void CRecvDlg::OnBnClickedCopyaddress()
{
	// TODO: 在此添加控件通知处理程序代码
	string StrShow;
	POSITION pos = m_listCtrl.GetFirstSelectedItemPosition() ;
...
void   CRecvDlg::ModifyListCtrlItem()
{
	CPostMsg postmsg;
...
void   CRecvDlg::InsertListCtrlItem()
{
	CPostMsg postmsg;
	if (!theApp.m_msgUIRecDlgQueue.pop(postmsg))
	{
...
LRESULT  CRecvDlg::OnShowListCtrl(  WPARAM wParam, LPARAM lParam )
{
	int type = (int)wParam;
	switch(type)
	{
...
void CRecvDlg::OnBnClickedExportExel2()
{
	// TODO: 在此添加控件通知处理程序代码
