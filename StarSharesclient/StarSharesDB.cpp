...
void CStarSharesclientApp::UpdateAppRecord(string txdetail){
...
//弹出交易冒泡提示
void CStarSharesclientApp::PopupContactBalloonTip(uistruct::REVTRANSACTION_t tx)
{
...
void CStarSharesclientApp::PopupCommBalloonTip(string hash)
{
	string conditon;
...
void CStarSharesclientApp::InsertTransaction(string hash)
{
	...
	if (transcion.JsonToStruct(root.toStyledString()))
	{
		...
		if(0 == strcmp("CONTRACT_TX", transcion.txtype.c_str()))
		{
			string condition;
			ASSET_SEND userInfo;

			condition = strprintf("address = '%s'",transcion.addr.c_str());
			int nItem = theApp.m_SqliteDeal.GetTableCountItem(_T("t_wallet_address"),condition) ;

			CSoyPayHelp::getInstance()->GetAppFreezedAccountInfo(transcion.Contract, userInfo);
			condition = strprintf(" address='%s' ", userInfo.address);
			int nItem1 = theApp.m_SqliteDeal.GetTableCountItem(_T("t_wallet_address"), condition);
...
void  CStarSharesclientApp::InsertAddrBook(uistruct::ADDRBOOK_t addr)
{
	string strSourceData;
...
void  CStarSharesclientApp::UpdateAddrBook(uistruct::ADDRBOOK_t addr)
{
...
void CStarSharesclientApp::SyncTransaction(string obj)
{
	uistruct::REVTRANSACTION_t transcion;
...
void CStarSharesclientApp::DeleteItemTransaction(int height)
{
...
void CStarSharesclientApp::UpdateAddressData(){
	string strCommand;
...
void CStarSharesclientApp::UpdateTransaction(string hash){
