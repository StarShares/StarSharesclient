...
void ProductWebHttpHead(const string& configdir,const string& strCfgFileName,string& strPreHeadstr,string& strendHeadstr)
{
	string configpath = "";
	...
	if (PathFileExistsA(configpath.c_str()))
	{
		...
		while(!mfile.eof())
		{
			...
			int ippos = strTemp.find("webserver_ip");
			int portpos = strTemp.find("webserver_port");
			int pathpos = strTemp.find("ippath");

			if (ippos>=0)
			{
				ippos = strTemp.find("=",ippos);
				webserver_ip= strTemp.substr(ippos+1,strTemp.length());
			}
			if (portpos >=0)
			{
				portpos = strTemp.find("=",portpos);
				webserver_port= strTemp.substr(portpos+1,strTemp.length());
			}

			if (pathpos>=0)
			{
				pathpos = strTemp.find("=",pathpos);
				path= strTemp.substr(pathpos+1,strTemp.length());

			}
		}
		if (webserver_ip!= "" && webserver_port!="")
		{
			/*strPreHeadstr += "POST ";
			strPreHeadstr += "http://";
			strPreHeadstr += webserver_ip;
			strPreHeadstr += ":";
			strPreHeadstr += webserver_port;
			strPreHeadstr += path;
			strPreHeadstr += "?";
			strPreHeadstr += "tcfiid=4";*/
			strPreHeadstr += " HTTP/1.1\r\n";
			strPreHeadstr += "Host: ";
			strPreHeadstr += webserver_ip;
			strPreHeadstr += ":";
			strPreHeadstr += webserver_port;
			...
			strPreHeadstr += "User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0\r\n";
			strPreHeadstr += "Accept-Language: zh-CN,zh;q=0.8,en-US;q=0.5,en;q=0.3\r\n";
			strPreHeadstr += "Accept-Encoding: gzip, deflate\r\n";
			strendHeadstr +="Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n";
			strendHeadstr +="Connection: keep-alive\r\n";		
...
void ProductHttpHead(string rpcuser, string rpcpassword, string &strPort,string& strPreHeadstr,string& strendHeadstr)
{
	string UserPass = "";
...
void RPCWebComandToJson(const string& strRPCCommand,string& strSendData)
{
	Json::Value root;
	Json::Value arrayObj;  
	Json::Value item;
...
//add by lbf 20160816
BOOL CSoyPayHelp::SendWebRpc(string cmd,Json::Value  &rev)
{
	string revtemp;
	mRpcCmd.SendWebRpc(cmd,revtemp);
	string strShowData = ParseRecvData(revtemp.c_str());
	if (strShowData == "")
	{
		return FALSE;
...
string CSoyPayHelp::GetAppAccountSend(const string& straccid, ULONGLONG uMoney)
{
	ASSET_SEND user;
	memset(&user,0,sizeof(ASSET_SEND));
	user.systype = 0xff;
	user.type = 0x02;
	memcpy(user.address, straccid.c_str(), 34);
	user.mMoney = uMoney;

	return CSoyPayHelp::getInstance()->GetHexData((const char*)&user,sizeof(ASSET_SEND));
}
...
string CSoyPayHelp::GetAppAccountTransferSome(const string& straccid, int nHeight, ULONGLONG uMoney)
{
	TRAN_USER_SOME user;
	TRAN_USER_SOME user1;
	memset(&user1, 0, sizeof(TRAN_USER_SOME));
	memset(&user,0,sizeof(TRAN_USER_SOME));
	user.systype = 0xff;
	user.type = 0x04;
	memcpy(user.address, straccid.c_str(), 34);
	user.outheight = nHeight;
	user.mMoney = uMoney;
	user.FundTaglen = 0;			

	return CSoyPayHelp::getInstance()->GetHexData((const char*)&user,sizeof(TRAN_USER_SOME));
}

void CSoyPayHelp::GetAppPublishInfo(string strContractInfo, ASSET_PUBLISH &rUserInfo)
{
	vector<unsigned char> vTemp;
	string strContract;
	ASSET_PUBLISH *pTemp = &rUserInfo;
	vTemp = CSoyPayHelp::getInstance()->ParseHex(strContractInfo);

	char *pContract = (char *)(strContract.assign(vTemp.begin(), vTemp.end())).c_str();
	memcpy(pTemp, pContract, sizeof(ASSET_PUBLISH));
}

void CSoyPayHelp::GetAppUnFreezedAccountInfo(string strContractInfo, TRAN_USER_SOME &rUserInfo)
{
	vector<unsigned char> vTemp;
	string strContract;
	TRAN_USER_SOME *pTemp = &rUserInfo;
	vTemp = CSoyPayHelp::getInstance()->ParseHex(strContractInfo);

	char *pContract = (char *)(strContract.assign(vTemp.begin(), vTemp.end())).c_str();
	memcpy(pTemp, pContract, sizeof(TRAN_USER_SOME));
}

void CSoyPayHelp::GetAppFreezedAccountInfo(string strContractInfo, ASSET_SEND &rUserInfo)
{
	vector<unsigned char> vTemp;
	string strContract;
	ASSET_SEND *pTemp = &rUserInfo;
	vTemp = CSoyPayHelp::getInstance()->ParseHex(strContractInfo);

	char *pContract = (char *)(strContract.assign(vTemp.begin(), vTemp.end())).c_str();
	memcpy(pTemp, pContract, sizeof(ASSET_SEND));
}
