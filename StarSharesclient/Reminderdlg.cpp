...
void  CReminderdlg::WriteClosConfig(int &close)
{
	if (PathFileExistsA(theApp.m_strInsPath.c_str()))
	{
		...
		strpathe +="\\StarSharesclient.conf";
...
void CReminderdlg::OnBnClickedOk()
{
	...
	if (((CButton *)GetDlgItem(IDC_RADIO_NOEXIT))->GetCheck())
	{
	   ...
	   ((CStarSharesclientDlg*)(theApp.m_pMainWnd))->ToTray();
...
HBRUSH CReminderdlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if(id == IDC_RADIO_NOEXIT || id == IDC_RADIO_EXIT || id == IDC_CHECK_NO)
	{
