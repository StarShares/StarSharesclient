...
#define MSG_USER_ASSET_SEND_UI				WM_USER+122		//更新资产转出
#define MSG_USER_ASSET_RECV_UI				WM_USER+123		//更新资产接收
...
namespace uistruct {
	...
	//认购的资产
	typedef struct BOUGHTASSET{   
		string    AssetID ;					//发行的资产ID
		string    AssetName   ;		        //资产名称
		string AssetIssuer;					//资产发行方
		double CurAmount;					//当前众筹总金额
		int    BProgress;					//认购进度
		int    iState;						//认购状态
		bool bResult;						//认购结果
		double SupportAmount;				//支持
		BOUGHTASSET(){
			AssetID="";
			AssetName="";
			AssetIssuer="";
		...
		string ToJson(){
			...
			root["AssetIssuer"] = AssetIssuer;
			root["CurAmount"] = CurAmount;
			root["BProgress"] = BProgress;
			root["iState"] = iState;
			root["bResult"] = bResult;
			root["SupportAmount"] = SupportAmount;
			return root.toStyledString();
		}
		bool JsonToStruct(string json){
			Json::Reader reader;  
			...
			if (!reader.parse(json, root)) 
				return false ;
			AssetID = root["AssetID"].asString();
			AssetName =  root["AssetName"].asString();
			AssetIssuer =  root["AssetIssuer"].asString();
			this->CurAmount = root["CurAmount"].asDouble();
			this->BProgress = root["BProgress"].asInt();
			this->iState = root["iState"].asInt();
			this->bResult = root["bResult"].asBool();
			this->SupportAmount = root["SupportAmount"].asDouble();

			return true;
		}
	}BOUGHTASSET_t;
	typedef std::vector<BOUGHTASSET_t> BOUGHTASSETLIST ;


	//资产转出
	typedef struct ASSETDEAL{   
		string    AssetID ;							//发行的资产ID
		string    AssetName   ;						//资产名称
		string    strRecvAssetAddr;					//接收资产地址
		string	  strSendAssetAddr;					//发送资产地址
		INT64	  popNum;							//支持
		int       confirmedHeight;					//确认高度
		int		  iState;							//转移状态
		ASSETDEAL(){
			AssetID="";
			AssetName="";
		...
		string ToJson(){
			...
			root["AssetID"] = AssetID;
			root["AssetName"] = AssetName;
			root["strRecvAssetAddr"] = strRecvAssetAddr;
			root["strSendAssetAddr"] = strSendAssetAddr;
			root["popNum"] = popNum;
		...
		bool JsonToStruct(string json){
			...
			AssetID = root["AssetID"].asString();
			AssetName =  root["AssetName"].asString();
			strRecvAssetAddr = root["strRecvAssetAddr"].asString();
			strSendAssetAddr = root["strSendAssetAddr"].asString();
			popNum = root["popNum"].asInt64();
			confirmedHeight = root["confirmedHeight"].asInt();
			iState = root["iState"].asInt();

			return true;
		}
	}ASSETDEAL_t;
	typedef std::vector<ASSETDEAL_t> ASSETDEALLIST ;



	//lbf
	//发行的所有资产
	typedef struct LISTASSET{   
		int    itcfiid;						//标签     
		string AssetID ;					//发行的资产ID
		string AssetName   ;		        //资产名称
		string AssetIssuer;					//资产发行方
		...
		time_t issueEndTime;				//结束日期
		string strUrlPic;					//资产图片
		string AssetAddr;					//发行的目的地址
		double CurAmount;					//当前众筹总金额

		string buyProgress;				//认购进度
		int status;						//认购状态
		bool bResult;						//认购结果
		...
		string ToJson(){
			Json::Value root;
			root["itcfiid"] = itcfiid;
			root["AssetID"] = AssetID;
			root["AssetName"] = AssetName;
			root["AssetIssuer"] = AssetIssuer;
			root["PopNum"] = PopNum;
			root["MaxNum"] = MaxNum;
			root["GoalNum"] = GoalNum;
			root["IssueStartTime"] = IssueStartTime;
			root["issueEndTime"] = issueEndTime;
			root["strUrlPic"] = strUrlPic;
			root["AssetAddr"] = AssetAddr;
			root["CurAmount"] = CurAmount;
			root["buyProgress"] = buyProgress;
			root["status"] = status;
			root["bResult"] = bResult;
		...
		bool JsonToStruct(string json){
			...
			if (!reader.parse(json, root)) 
				return false ;
			itcfiid				 =  root["itcfiid"].asInt();
			AssetID				 =  root["AssetID"].asString();
			AssetName			 =  root["AssetName"].asString();
			AssetIssuer			 =  root["AssetIssuer"].asString();
			PopNum				 =  root["PopNum"].asDouble();
			MaxNum				 =  root["MaxNum"].asDouble();
			GoalNum				 =  root["GoalNum"].asDouble();
			IssueStartTime		 =  root["IssueStartTime"].asInt();
			issueEndTime		 =  root["issueEndTime"].asInt();
			strUrlPic			 =  root["strUrlPic"].asString();
			AssetAddr			 =  root["AssetAddr"].asString();
			CurAmount			 =  root["CurAmount"].asDouble();
			buyProgress          =  root["buyProgress"].asString();
			status               =  root["status"].asInt();
			bResult				 =  root["bResult"].asBool();

			return true;
		}
	}LISTASSET_t;
	typedef std::vector<LISTASSET_t> ASSETLIST ;

	//资产对应的购买配置参数

	typedef struct LISTASSETPARAM{   
		int    itcfiid;						//标签     
		string AssetID ;					//发行的资产ID
		int    iSubid;						//子ID
		double damount;						//资产购买参数
		...
		bool JsonToStruct(string json){
			...
			itcfiid				 =  root["itcfiid"].asInt();
			AssetID				 =  root["AssetID"].asString();
			iSubid				 =  root["iSubid"].asInt();
			damount				 =  root["damount"].asDouble();

			return true;
		}
	}LISTASSETPARAM_t;
	typedef std::vector<LISTASSETPARAM_t> ASSETPARAMLIST ;

	//我的资产
	typedef struct LISTMYASSET{
		string    assetId ;			//资产ID
		string    assetName   ;		//资产名称
		string    issuer ;			//发行方
		string	  assetAddr   ;		//资产对应的地址
		double    fMoney      ;     //数量
		LISTMYASSET()
		...
		bool JsonToStruct(string json){
			...
			assetId = root["assetId"].asString();
			assetName = root["assetName"].asString();
			issuer =  root["issuer"].asString();
			assetAddr = root["assetAddr"].asString();
			fMoney = root["fMoney"].asDouble();

			return true;
		}
	}LISTMYASSET_t;
	typedef std::vector<LISTMYASSET_t> LISTMYASSETLIST ;
	//lbf
