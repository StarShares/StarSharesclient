...
IMPLEMENT_DYNAMIC(CProgStatusDlg, CDialogBar)

CProgStatusDlg::CProgStatusDlg()
{
	m_pBmp = NULL ;
...
void CProgStatusDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CProgStatusDlg, CDialogBar)
	ON_WM_ERASEBKGND()
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_MESSAGE(MSG_USER_UP_PROGRESS , &CProgStatusDlg::OnShowProgressCtrl  )
	ON_WM_PAINT()
	ON_WM_MOUSEMOVE()
END_MESSAGE_MAP()
...
// CProgStatusDlg 消息处理程序

void CProgStatusDlg::SetBkBmpNid( UINT nBitmapIn ) 
{
	if( NULL != m_pBmp ) {
		::DeleteObject( m_pBmp ) ;
		m_pBmp = NULL ;
...
void CProgStatusDlg::LoadGifing( BOOL bState )
{
	if( NULL != m_ProgressWnd ) {
...
BOOL CProgStatusDlg::OnEraseBkgnd(CDC* pDC)
{
...
int CProgStatusDlg::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CProgStatusDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	{
		pst->GetWindowRect(rect);
		pst->SetWindowPos( NULL ,Width - 5 , 10, rect.Width(), rect.Height()  ,SWP_SHOWWINDOW ) ; 
		Width += rect.Width();
	}
	...
	if(NULL != pst)
	{
		...
		pst->SetWindowPos( NULL ,900 - 80 - nVerWidth - 5, 13, rect.Width(), rect.Height()  ,SWP_SHOWWINDOW ) ; 
	}
...
BOOL CProgStatusDlg::Create(CWnd* pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID)
{
...
int CProgStatusDlg::ShowProgressCtrl(){
	CPostMsg postmsg;
...
void CProgStatusDlg::ShowLockCtrl()
{
	CPostMsg postmsg;
...
void CProgStatusDlg::ShowNetCount()
{
	CPostMsg postmsg;
...
void CProgStatusDlg::OnPaint()
{
	...
	dc.BitBlt(900-50, 6, rc.Width(), rc.Height(), &memDC, 0, 0, SRCCOPY);  
	CRect rc1(900-50, 6, rc.Width(), rc.Height());
	m_rectSignal = rc1;
...
//	memDC.DeleteDC();  
	if (theApp.m_bIsHaveLocked)
	{
		...
		dc.BitBlt(900-50-20, 8, rc.Width(), rc.Height(), &memDC, 0, 0, SRCCOPY);  
		CRect rc2(900-50-20, 8, rc.Width(), rc.Height());
		m_rectLock = rc2;
		memDC.SelectObject(hlockbmp);  
	}
	memDC.DeleteDC();  
}
void CProgStatusDlg::OnIniLockParam()
{
	string strCommand;
...
void CProgStatusDlg::SetAppStepfee(int fuelrate)
{
	if (fuelrate == 0)
	{
		return;
...
void CProgStatusDlg::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CRect temp = m_rectSignal;
	//ClientToScreen(&temp);
	CStarSharesclientDlg *pDlg = (CStarSharesclientDlg*)(theApp.m_pMainWnd);
...
string CProgStatusDlg::GetUIVersion()
{
	string strRet = "";
