...
IMPLEMENT_DYNAMIC(CRPCDlg, CBaseDlg)

CRPCDlg::CRPCDlg(CWnd* pParent /*=NULL*/)
	: CBaseDlg(CRPCDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CRPCDlg, CBaseDlg)
...
BOOL CRPCDlg::OnInitDialog()
{
	...
	*/
	...
	m_rBtnClear.SetBitmaps( IDB_BITMAP_QK2 , RGB(255, 255, 0) , IDB_BITMAP_QK1 , RGB(255, 255, 255) );
	...
	*/
	...
	m_rBtnClear.SetColor(CButtonST::BTNST_COLOR_FG_IN , RGB(255,255, 255));
	...
	configpath.AppendFormat("\\%s","StarSharesclient.conf");
...
void CRPCDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if( NULL != GetSafeHwnd() ) {
		...
		if ( NULL != pst ) {
			CRect rect ;
			pst->GetClientRect( rect ) ;
			pst->SetWindowPos( NULL , 75*rc.Width()/div  , 95*rc.Height()/div - 2, rect.Width(), rect.Height(),SWP_SHOWWINDOW ) ;
		}
	}
}
