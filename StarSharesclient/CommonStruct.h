...
#define WINDOW_TAG				_T("StarShares-UIDemo")/*_T("harrypoints-UIDemo")*/
...
typedef struct {
	...
	ULONGLONG money;               //  总金额 >= freemoney + freeMothmoney * (FREEZE_MONTH_NUM - 1)
	ULONGLONG freemoney;           //  自由金额
	ULONGLONG freeMothmoney;       //  每个月能领的金额(由冻结金额，转为自由金额)
}IPO_USER;
...
typedef struct {
	unsigned char systype;		//ff
	unsigned char type;			//04
	unsigned char address[34]; //  转账地址
	unsigned int outheight;		    //!< the transacion Timeout height
	ULONGLONG mMoney;			        //!<The transfer amount
...
typedef struct {
	unsigned char systype;
	unsigned char type;
	unsigned char address[34]; //  发行地址

	ULONGLONG mMoney;//发行总量
	ULONGLONG ullCrowdfunding;//众筹量
...
public:
   //CRPCDlg * pdlg,
	...
	//web端Rpc初始化2016/08/16
	void InitialWebRpcCmd(const string &webSeverIp,const string	 &sendWebPreHeadstr,const string &sendWebEndHeadstr,const string &webServerPort)
	{
		mRpcCmd.SetWebHeadString(sendWebPreHeadstr,sendWebEndHeadstr);
		mRpcCmd.SetWebPort(atoi(webServerPort.c_str()));
		mRpcCmd.SetWebSeverIp(webSeverIp);
	} 
	BOOL SendWebRpc(string cmd,Json::Value  &rev);
	...
	//lbf 2016/07/15
	string GetAppAccountTransferSome(const string& straccid, int nHeight, ULONGLONG uMoney);
	void GetAppPublishInfo(string strContractInfo, ASSET_PUBLISH &rUserInfo);
	void GetAppUnFreezedAccountInfo(string strContractInfo, TRAN_USER_SOME &rUserInfo);
	string GetAppAccountSend(const string& straccid, ULONGLONG uMoney);
	void GetAppFreezedAccountInfo(string strContractInfo, ASSET_SEND &rUserInfo);
...
extern void ProductHttpHead(string rpcuser, string rpcpassword, string &strPort,string& strPreHeadstr,string& strendHeadstr);
...
//add by lbf20160801
extern void ProductWebHttpHead(const string& configdir,const string& strCfgFileName,string& strPreHeadstr,string& strendHeadstr);
