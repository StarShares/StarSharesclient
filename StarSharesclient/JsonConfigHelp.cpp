...
void CJsonConfigHelp::Init()
{
		...
		memset(&m_WebParam,0,sizeof(CWebParamCfg));
...
void CJsonConfigHelp::ReadJsonConfig(const string& strFilePath)
{
	...
	if (reader.parse(ifs, root,false))
	{
		...
		pos = strTemp.find("webparam");
		if (pos > 0)
		{
			ReadWebParmCfgData(root);
		}else{
			UiFun::MessageBoxEx(_T("webparam Œ¥≈‰÷√") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(0);
		}
...
void CJsonConfigHelp::AddString(const Json::Value& root,CAutoComplete &m_comboxinput){
	...
	m_comboxinput.GetStringList().Add(rpccommand["notionalpoolingasset"].asCString());
}
...
void CJsonConfigHelp::ReadWebParmCfgData(const Json::Value& root){
	Json::Value webparam = root["webparam"];
	ASSERT(!webparam.isNull());
	ASSERT(!webparam["webserver_ip"].isNull());
	Json::Value serverIpValue = webparam["webserver_ip"];
	m_WebParam.webserver_ip = webparam["webserver_ip"].asString();
	m_WebParam.webserver_port= webparam["webserver_port"].asString();
	m_WebParam.web_fullpath = webparam["fullpath"].asString();
	m_WebParam.web_singlepath = webparam["singlepath"].asString();
}

void CJsonConfigHelp::GetWebParamCfgData(CWebParamCfg& webparm)
{
	webparm = m_WebParam;
}
