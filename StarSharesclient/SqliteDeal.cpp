...
BOOL CSqliteDeal::InitializationDB(){
	...
	//创建发行的资产数据表LBF2016/7/22
	strCondition = "type = 'table' and name = 't_asset'";
	if(!GetTableCountItem(strTableName, strCondition))
	{
		string createSQL = "CREATE TABLE t_asset(ID INT PRIMARY KEY, AssetID TEXT, AssetName TEXT, AssetIssuer TEXT, PopNum DOUBLE, MaxNum DOUBLE, GoalNum DOUBLE, IssueStartTime INT, IssueEndTime INT, strUrlPic TEXT, AssetAddr TEXT, CurrentAmount DOUBLE,BuyProgress TEXT, Status INT, Result INT)";
		if(!ExcuteSQL(pDBConn, NULL, createSQL, NULL))
		{
			LogPrint("INFO", "Create table t_asset failed\n");
			return FALSE;
		}
	}

	//创建资产对应的认购人气币参数
	strCondition = "type = 'table' and name = 't_asset_param'";
	if(!GetTableCountItem(strTableName, strCondition))
	{
		string createSQL = "CREATE TABLE t_asset_param(ID INT, AssetID TEXT, SubID INT, Amount DOUBLE)";
		if(!ExcuteSQL(pDBConn, NULL, createSQL, NULL))
		{
			LogPrint("INFO", "Create table t_asset_param failed\n");
			return FALSE;
		}
	}
...
//获取我的某一个资产
int CallGetMyAssetItem(void *para, int n_column, char ** column_value, char ** column_name)
{
	uistruct::LISTMYASSET_t *pAssetAddr = (uistruct::LISTMYASSET_t *)para;
	if(NULL == column_value[0])
		return -1;
	if(n_column != 5)
		return -1;

	pAssetAddr->assetId = strprintf("%s",column_value[0]);

	pAssetAddr->assetName = strprintf("%s",column_value[1]);

	pAssetAddr->issuer = strprintf("%s", column_value[2]);

	pAssetAddr->assetAddr = strprintf("%s", column_value[3]) ;

	pAssetAddr->fMoney = strtod(column_value[4],NULL);
...
//获取我的资产列表
int CallGetMyAssetList(void *para, int n_column, char ** column_value, char ** column_name)
{
	map<string,uistruct::LISTMYASSET_t> *pListInfo = (map<string,uistruct::LISTMYASSET_t> *)para;
	if(n_column != 5)
		return -1;

	uistruct::LISTMYASSET_t listdata;

	if(CallGetMyAssetItem(&listdata, n_column, column_value, column_name) < 0 )
		return -1;

	(*pListInfo)[listdata.assetId] = listdata;
	return 0;
}

//获取我的某一个资产
int CallGetAssetItem(void *para, int n_column, char ** column_value, char ** column_name)
{
	uistruct::LISTASSET_t *pAssetAddr = (uistruct::LISTASSET_t *)para;
	if(NULL == column_value[0])
		return -1;
	if(n_column != 15)
		return -1;

	pAssetAddr->itcfiid = atoi(column_value[0]);
	pAssetAddr->AssetID = strprintf("%s",column_value[1]);
	pAssetAddr->AssetName = strprintf("%s",column_value[2]);
	pAssetAddr->AssetIssuer = strprintf("%s", column_value[3]);
	pAssetAddr->PopNum = (INT64)(strtod ( column_value[4] , NULL));
	pAssetAddr->MaxNum = (INT64)(strtod ( column_value[5] , NULL));
	pAssetAddr->GoalNum = (INT64)(strtod ( column_value[6] , NULL));

	stringstream strValue;
	strValue << strprintf("%s",column_value[7]);
	UINT64  value = 0;
	strValue >> value;
	pAssetAddr->IssueStartTime = value;
	strValue.clear();
	strValue.str("");
	strValue << strprintf("%s", column_value[8]);
	strValue >> value;
	pAssetAddr->issueEndTime = value;
	strValue.clear();
	strValue.str("");
	pAssetAddr->strUrlPic = strprintf("%s", column_value[9]);
	pAssetAddr->AssetAddr = strprintf("%s", column_value[10]);
	pAssetAddr->CurAmount = (INT64)(strtod ( column_value[11] , NULL));
	pAssetAddr->buyProgress = strprintf("%s", column_value[12]);
	pAssetAddr->status = atoi(column_value[13]);
	pAssetAddr->bResult = atoi(column_value[14]);

	return 0;
}

int CallGetAssetParamItem(void *para, int n_column, char ** column_value, char ** column_name)
{
	uistruct::LISTASSETPARAM_t *pAssetAddr = (uistruct::LISTASSETPARAM_t *)para;
	if(NULL == column_value[0])
		return -1;
	if(n_column != 4)
		return -1;
	...
	pAssetAddr->itcfiid = atoi(column_value[0]);
	pAssetAddr->AssetID = strprintf("%s",column_value[1]);
	pAssetAddr->iSubid = atoi(column_value[2]);
	pAssetAddr->damount = (INT64)(strtod ( column_value[3] , NULL));

	return 0;
}

//获取资产列表
int CallGetMapAssetList(void *para, int n_column, char ** column_value, char ** column_name)
{
	map<string,uistruct::LISTASSET_t> *pListInfo = (map<string,uistruct::LISTASSET_t> *)para;
	if(n_column != 15)
		return -1;

	uistruct::LISTASSET_t listdata;

	if(CallGetAssetItem(&listdata, n_column, column_value, column_name) < 0 )
		return -1;
...
int CallGetVecAssetList(void *para, int n_column, char ** column_value, char ** column_name)
{
	uistruct::ASSETLIST *pListInfo = (uistruct::ASSETLIST *)para;
	if(n_column != 15)
		return -1;
...
int CallGetVecAssetParamList(void *para, int n_column, char ** column_value, char ** column_name)
{
	uistruct::ASSETPARAMLIST *pListInfo = (uistruct::ASSETPARAMLIST *)para;
	if(n_column != 4)
		return -1;

	uistruct::LISTASSETPARAM_t listdata;

	if(CallGetAssetParamItem(&listdata, n_column, column_value, column_name) < 0 )
		return -1;
...
int CSqliteDeal::GetAssetItem(const string &strCond, uistruct::LISTASSET_t *pAsset)
{
	sqlite3 ** pDBConn = GetDBConnect(); //获取数据库连接

	string strSQL="";
	strSQL = "SELECT * FROM t_asset WHERE " + strCond;
	ExcuteSQL(pDBConn, &CallGetAssetItem, strSQL, (void *)(pAsset));
	return 0;
}

//lbf
//int        CSqliteDeal::GetMyAssetItem(const string &strCond, uistruct::LISTMYASSET_t *pMyAsset)
//{
//	sqlite3 ** pDBConn = GetDBConnect(); //获取数据库连接
//
//	string strSQL="";
//	strSQL = "SELECT * FROM t_my_asset WHERE " + strCond;
//	ExcuteSQL(pDBConn, &CallGetMyAssetItem, strSQL, (void *)(pMyAsset));
//	return 0;
//}
...
//int CSqliteDeal::GetMyAssetList(const string &strCondition, map<string,uistruct::LISTMYASSET_t> *pListInfo)		//lbf
//{
//	sqlite3 ** pDBConn = GetDBConnect();
//	string strSQL("");
//	strSQL = "SELECT * FROM t_my_asset";
//	ExcuteSQL(pDBConn, &CallGetMyAssetList, strSQL, (void *)pListInfo);
//	return 0;
//}

int	CSqliteDeal::GetAssetList(const string &strCondition, map<string,uistruct::LISTASSET_t> *pListInfo)
{
	sqlite3 ** pDBConn = GetDBConnect();
	string strSQL("");
	strSQL = "SELECT * FROM t_asset WHERE " + strCondition;
	ExcuteSQL(pDBConn, &CallGetMapAssetList, strSQL, (void *)pListInfo);
	return 0;
}

int	CSqliteDeal::GetAssetParamList(const string &strCondition, uistruct::ASSETPARAMLIST *pAssetList)
{
	sqlite3 ** pDBConn = GetDBConnect(); //获取数据库连接
	string strSQL("");
	strSQL= strprintf("SELECT * FROM t_asset_param WHERE %s",(LPSTR)(LPCTSTR)strCondition.c_str());
	return ExcuteSQL(pDBConn , &CallGetVecAssetParamList, strSQL, (void*)(pAssetList));
}

int	CSqliteDeal::GetAssetList(const string &strCondition, uistruct::ASSETLIST *pAssetList)
{
	sqlite3 ** pDBConn = GetDBConnect(); //获取数据库连接
	string strSQL("");
	strSQL= strprintf("SELECT * FROM t_asset WHERE %s",(LPSTR)(LPCTSTR)strCondition.c_str());
	return ExcuteSQL(pDBConn , &CallGetVecAssetList, strSQL, (void*)(pAssetList));
}
