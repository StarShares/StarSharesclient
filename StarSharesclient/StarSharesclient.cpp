...
// StarSharesclient.cpp : 定义应用程序的类行为。
//
...
// CStarSharesclientApp

BEGIN_MESSAGE_MAP(CStarSharesclientApp, CWinApp)
	ON_COMMAND(ID_HELP, &CWinApp::OnHelp)
END_MESSAGE_MAP()


// CStarSharesclientApp 构造

CStarSharesclientApp::CStarSharesclientApp()
{
	// 支持重新启动管理器
	...
	m_strServerCfgFileName = "StarShares.conf";  //服务器配置文件名
...
// 唯一的一个 CStarSharesclientApp 对象

CStarSharesclientApp theApp;


// CStarSharesclientApp 初始化
Gdiplus::GdiplusStartupInput m_gdiplusStartupInput;
ULONG_PTR m_pGdiToken;

BOOL CStarSharesclientApp::InitInstance()
{
	...
	ProductWebHttpHead(m_strInsPath, m_strServerCfgFileName, m_webSendPreHeadstr, m_webSendEndHeadstr);
...
BOOL CStarSharesclientApp::RunOnlyOneApp()
{
	HANDLE hMutex = CreateMutex(NULL, FALSE, _T("StarSharesclient"));
	// 检查错误代码
...
BOOL CStarSharesclientApp::CreateMaintainThrd() 
{
	hMtThrd		= NULL ;
	nMtThrdID	= 0  ;
...
UINT __stdcall CStarSharesclientApp::MtProc(LPVOID pParam)
{
	MSG	msg , msgcpy ;
	PeekMessage(&msg, NULL, WM_USER, WM_USER, PM_NOREMOVE) ;

	if(!SetEvent( ((CStarSharesclientApp*)pParam)->hMtStartEvent ) ) {
		return 1;
	}
	...
	{	
		...
		if ( MsgReturn )
		{
			switch ( msg.message )
			{
			case MSG_USER_MOBILE_STATEUS:					//切换界面时::PostThreadMessage( theApp.GetMtHthrdId() , MSG_USER_MOBILE_STATEUS , CMainDlg::IDD , 0 ) ;
				{
                    ::PostThreadMessage( ((CStarSharesclientApp*)pParam)->GetMtHthrdId() , MSG_USER_MOBILE_TIP , msgcpy.wParam , msgcpy.lParam ) ;
				}
				break;
			case MSG_USER_QUITTHREAD:
				{
					std::vector< sThrd >::iterator it ;
					CStarSharesclientApp* pApp = ((CStarSharesclientApp*)pParam) ;
			...
			((CStarSharesclientApp*)pParam)->DispatchMsg( ((CStarSharesclientApp*)pParam)->GetMtHthrdId() , msgcpy.message , msgcpy.wParam , msgcpy.lParam ) ;
		}
	}
}
void CStarSharesclientApp::RegThrdToMtThrd( HANDLE hThread , unsigned int hThrdId )
{
...
void CStarSharesclientApp::UnRegThrdToMtThrd( HANDLE hThread , unsigned int hThrdId ) 
{
	if( NULL == hThrdId || NULL == hThread ) {
		return ;
	}
...
void CStarSharesclientApp::SubscribeMsg( DWORD nThreadId , HWND hWnd, UINT msg , void * relMsgMem )
{
	if( NULL == hWnd ) {
		return ;
	}
...
void CStarSharesclientApp::UnSubscribeMsg( DWORD nThreadId , HWND hWnd, UINT msg )
{
	if( NULL == hWnd ) {
		return ;
	}
...
void CStarSharesclientApp::DispatchMsg( unsigned int threadID , UINT msg , WPARAM wParam , LPARAM lParam ) 
{
	BOOL flag = FALSE ;
	std::vector<MSG_NOTIFY>::iterator it ;
...
void CStarSharesclientApp::GetMoFilename( string & path , string & filename ) 
{
...
void CStarSharesclientApp::StartSeverProcess(const string& strdir)
{
	CloseProcess(_T("StarSharesd.exe"));
	...
	string startBat = strprintf("StarSharesd.exe -datadir=%s -ui=1", strdir);
	if(m_bReIndexServer)
...
void CStarSharesclientApp::CloseProcess(const string& exename)
{
...
//todo 判断钱包是否已加密
bool CStarSharesclientApp::IsLockWallet(){
...
void CStarSharesclientApp::OnCheckSeverfile()
{
	string strSeverPath = strprintf("%s\\StarSharesd.exe", m_strInsPath);
	...
	if (PathIsDirectory(strSeverPath.c_str()))
	{
		...
		{
			UiFun::MessageBoxEx(_T("可执行StarSharesd.exe文件不存在,请重新启动钱包\r\n") , _T("Error") ,MFB_OK|MFB_ERROR );
			exit(1);
		}
	}

	strSeverPath = strprintf("%s\\temp\\StarSharesd.exe",m_strInsPath);
	...
	//if( (_access( strSeverPath.c_str(), 0 )) == -1 )
	//{
	//	UiFun::MessageBoxEx(_T("StarSharesd 文件不存在,请重新下载\r\n") , _T("Error") ,MFB_OK|MFB_ERROR );
	//	exit(1);
	//}
}

//校验路径合法性
void CStarSharesclientApp::CheckPathValid(const string& strDir)
{
	BOOL bExist = FALSE;
...
void CStarSharesclientApp::StartWorkThread() 
{
...
UINT __stdcall CStarSharesclientApp::ReceiveMessageFromServer(LPVOID pParam)
{
	bool bReConnect(false);
	int nMaxbufferLen(10*1024-1);
	CStarSharesclientApp * ppoppointsApp  = (CStarSharesclientApp*)pParam ;
	if ( NULL != ppoppointsApp ) {
		while (true)
		{
			Sleep(100);
			...
			if(bReConnect) {
				...
				CSynchronousSocket te;
				SOCKET nSocket = te.OnblockConnnect((char*)ppoppointsApp->m_severIp.c_str(),atoi(ppoppointsApp->m_uiPort.c_str()));
				if( INVALID_SOCKET != nSocket) {
					ppoppointsApp->m_uiConnSocket = nSocket;
					bReConnect = false;
				...
				if ( INVALID_SOCKET != ppoppointsApp->m_uiConnSocket ) {
					char  cRecvbuffer[10*1024];
					int nRecLen = recv( ppoppointsApp->m_uiConnSocket , cRecvbuffer , nMaxbufferLen , 0);
					...
					if ( nRecLen > 0 ) {
						...
						TRACE("\n");*/
						if(!ppoppointsApp->m_noUiMsgBuffer.AddBytesToBuffer(cRecvbuffer, nRecLen)) {
							if (INVALID_SOCKET != ppoppointsApp->m_uiConnSocket)
							{
								closesocket(ppoppointsApp->m_uiConnSocket);
								ppoppointsApp->m_uiConnSocket = INVALID_SOCKET;
							}
							ppoppointsApp->m_noUiMsgBuffer.ClearBuffer();
							bReConnect = true;
						}
					}
					else if(nRecLen == 0) 
...
bool ProcessMsgJson(Json::Value &msgValue, CStarSharesclientApp* pApp) 
{
...
UINT __stdcall CStarSharesclientApp::PraseJsonToMessage(LPVOID pParam)
{
	CStarSharesclientApp * ppoppointsApp  = (CStarSharesclientApp*)pParam;
	if ( NULL != ppoppointsApp ) {
		while (true)
		{
		    Sleep(100); 
			if (theApp.m_bIsCloseAppFlag && !ppoppointsApp->m_noUiMsgBuffer.HaveNoUiMsg())
			{
				return 1;
			}
			while(ppoppointsApp->m_noUiMsgBuffer.HaveNoUiMsg()) {
				string strMsg;
				ppoppointsApp->m_noUiMsgBuffer.GetNoUiMsg(strMsg);
				Json::Reader reader;  
				...
				if (!reader.parse(strMsg, jsonValue)) 
					continue;
				ProcessMsgJson(jsonValue, ppoppointsApp);
			}
		}
	}
	return 1;
...
bool CStarSharesclientApp::Update()
{
			CString sPath;
		...
		/*	if ( IDYES != UiFun::MessageBoxEx(sMsg, UiFun::UI_LoadString("COMM_MODULE" , "COMM_TIP" ,theApp.gsLanguage) , MFB_YESNO|MFB_TIP ) ){
			...
			if(m_pMainWnd != NULL)
				SendMessage(((CStarSharesclientDlg*)(m_pMainWnd))->m_hWnd,MSG_USER_CLOSEPROCESS,NULL,NULL);
			CloseProcess("StarSharesd.exe");
			DWORD exc;
			...
			ShRun.nShow = SW_SHOW;
			ShellExecuteEx(&ShRun);
			CloseProcess("StarSharesclient.exe");
			return true;
}


UINT __stdcall CStarSharesclientApp::CheckUpdate(LPVOID lpParam){

	CStarSharesclientApp * pApp  = (CStarSharesclientApp*)lpParam; 
...
UINT __stdcall CStarSharesclientApp::ProcessMessage(LPVOID pParam) {
	CStarSharesclientApp * pApp  = (CStarSharesclientApp*)pParam ;
	...
	while ( true)
	{
		...
		CStarSharesclientDlg *pAppDlg = (CStarSharesclientDlg*)(pApp->m_pMainWnd);
				...
				case WM_UP_BlLOCKTIP:
					{
						...
						if ( pApp->m_SqliteDeal.ClearTableData(_T("t_chain_tip") ) ) {
							...
							if (block.JsonToStruct(data))
							{
								...
								strCommand = strprintf("%s %s",_T("getblock") ,block.hash );

								if(!CSoyPayHelp::getInstance()->SendRpc(strCommand,root))
								{
								TRACE("getblock rpccmd error");
								}
								...
								{
									pApp->UpdateTransaction(arrayobj[i].asString());
								}

								//pApp->UpdateTransaction(block.hash);
								string strinsert;
...
void CStarSharesclientApp::SendUIMessage(int message, string data)
{
	...
	m_msgUIAssetOutDlgQueue.push(Postmsg);
	m_msgUIAssetInDlgQueue.push(Postmsg);
	...
	CPostMsg msg2(MSG_USER_ASSET_SEND_UI, message);
	m_msgQueue.push(msg2);

	CPostMsg msg3(MSG_USER_ASSET_RECV_UI, message);
	m_msgQueue.push(msg3);
}


int CStarSharesclientApp::SendPostThread(DWORD msgtype)
{
	CStarSharesclientDlg *pDlg = (CStarSharesclientDlg*)(((CStarSharesclientApp*)this)->m_pMainWnd) ;
	if (pDlg == NULL)
	{
		return 0 ;
	}
...
void CStarSharesclientApp::GetMainDlgStruct()
{
...
void CStarSharesclientApp::InsertAppTransaction(string txdetail)
{
...
void  CStarSharesclientApp::ParseUIConfigFile(const string& strExeDir){
	string configpath = "";
	...
	if (PathFileExistsA(configpath.c_str()))
	{
		...
		*/
		...
		CWebParamCfg webParm;
		
		CJsonConfigHelp::getInstance()->GetClosConfig(m_reminder);
		CJsonConfigHelp::getInstance()->GetpopTipsConfig(m_poptips);
		CJsonConfigHelp::getInstance()->GetNetParamCfgData(netParm);
		CJsonConfigHelp::getInstance()->GetWebParamCfgData(webParm);
		CJsonConfigHelp::getInstance()->GetListAppConfig(m_listapp);
		...
		m_webserverIp = webParm.webserver_ip;
		m_webserverPort = webParm.webserver_port;
		m_webFullPath = webParm.web_fullpath;
		m_webSinglePath = webParm.web_singlepath;
	}
}


int CStarSharesclientApp::ExitInstance()
{
