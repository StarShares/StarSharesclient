...
// StarSharesclient.h : PROJECT_NAME 应用程序的主头文件
//
...
// 有关此类的实现，请参阅 StarSharesclient.cpp
//
#include "uiapi.h"
...
#include < afxinet.h>
#include "UiHead.h"
using namespace uistruct;

#define strprintf tfm::format

using namespace std;

class CStarSharesclientApp : public CWinApp
{
public:
	CStarSharesclientApp();
...
public:
	...
	string		 m_webserverIp;			   //网站服务器IP
	string		 m_webserverPort;		   //网站服务器端口
	string		 m_webFullPath;			   //所有资产的路径
	string       m_webSinglePath;		   //单条资产的路径
	string       m_webSendPreHeadstr;	   //发送WEBRPC命令报文头
	string		 m_webSendEndHeadstr;	   //发送WEBRPC命令报文尾

	SOCKET       m_uiConnWebSocket;		   //UI与WEB服务器通信socket
...
public:
	...
	CMyQueue m_msgUIAssetOutDlgQueue;		//资产转出界面消息队列
	CMyQueue m_msgUIAssetInDlgQueue;		//资产转入界面消息队列
...
public:
	...
	map<unsigned int, LISTASSET_t> m_listAsset;
...
extern CStarSharesclientApp theApp;